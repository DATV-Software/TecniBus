...
export default function EditarEstudianteScreen() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (est.id_padre && est.padre) {
        const padre = padresData.find((p) => p.id === est.id_padre);
        if (padre) setPadreSeleccionado(padre);
      }
      if (est.id_parada && est.parada) {
        const parada = paradasData.find((p) => p.id === est.id_parada);
        if (parada) setParadaSeleccionada(parada);
      }
  ...
  const handleUpdate = async () => {
    ...
    if (success) {
      setToast({ visible: true, message: "Estudiante actualizado", type: "success" });
      setTimeout(() => router.back(), 1500);
    } else {
      setToast({ visible: true, message: "No se pudo actualizar", type: "error" });
    }
  };
    ...
    showAlert({ title: "Confirmar eliminación", message: "¿Eliminar este estudiante? Esta acción no se puede deshacer.", type: "warning", buttons: [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            setLoading(false);
            if (success) {
              setToast({ visible: true, message: "Estudiante eliminado", type: "success" });
              setTimeout(() => router.back(), 1500);
            } else {
    ...
    <View style={{ flex: 1, backgroundColor: Colors.tecnibus[50] }}>
      ...
      >
        <View style={{ backgroundColor: "#fff", borderRadius: 16, padding: 20, shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3, elevation: 2 }}>
          ...
          <FormField label="Apellido" icon={User} required placeholder="Ej: Pérez" value={apellido} onChangeText={setApellido} autoCapitalize="words" />
          ...
          <View style={{ marginBottom: 16 }}>
            ...
            <TouchableOpacity
              ...
              style={{ backgroundColor: "#F9FAFB", borderRadius: 12, paddingHorizontal: 14, paddingVertical: 14, flexDirection: "row", alignItems: "center", justifyContent: "space-between", borderWidth: 1.5, borderColor: "#E5E7EB" }}
            >
              {padreSeleccionado ? (
                <View style={{ flexDirection: "row", alignItems: "center", flex: 1 }}>
                  <User size={18} color={Colors.tecnibus[600]} strokeWidth={2} />
                  <Text style={{ fontSize: 15, color: "#1F2937", marginLeft: 10 }}>{padreSeleccionado.nombreCompleto}</Text>
                </View>
              ) : (
          ...
          {/* Parada */}
          <View style={{ marginBottom: 16 }}>
            <Text style={{ fontSize: 13, fontWeight: "600", color: "#374151", marginBottom: 6 }}>Parada (Opcional)</Text>
            <TouchableOpacity
              ...
              {paradaSeleccionada ? (
                <View style={{ flex: 1 }}>
                  <View style={{ flexDirection: "row", alignItems: "center" }}>
                    ...
                    <Text style={{ fontSize: 15, color: "#1F2937", marginLeft: 10 }}>{paradaSeleccionada.nombre || "Sin nombre"}</Text>
                  </View>
                  {paradaSeleccionada.ruta && <Text style={{ fontSize: 12, color: "#6B7280", marginLeft: 28, marginTop: 2 }}>Ruta: {paradaSeleccionada.ruta.nombre}</Text>}
                </View>
              ) : (
        ...
        >
          {loading ? (
            ...
            <>
              ...
              <Text style={{ color: "#fff", fontWeight: "700", fontSize: 16, marginLeft: 8 }}>Actualizar Estudiante</Text>
            </>
          )}
        </TouchableOpacity>
      </KeyboardAwareScrollView>
          ...
          <View style={{ backgroundColor: "#fff", borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: keyboardHeight > 0 ? 320 : "80%" }}>
            <View style={{ padding: 20, borderBottomWidth: 1, borderBottomColor: "#E5E7EB" }}>
              ...
              <View style={{ backgroundColor: "#F9FAFB", borderRadius: 10, flexDirection: "row", alignItems: "center", paddingHorizontal: 12, paddingVertical: 8 }}>
                <Search size={18} color="#9CA3AF" strokeWidth={2} />
                <TextInput style={{ flex: 1, marginLeft: 8, fontSize: 15, color: "#1F2937" }} placeholder="Buscar padre..." placeholderTextColor="#9CA3AF" value={searchPadre} onChangeText={setSearchPadre} autoCapitalize="words" />
              </View>
            </View>
            <FlatList
              data={filteredPadres}
