...
import { Check, Eye, EyeOff, Lock, Mail, Trash2, User, UserCircle } from "lucide-react-native";
...
export default function EditarConductorScreen() {
  ...
  const handleUpdate = async () => {
    ...
    if (result.success) {
      setToast({ visible: true, message: "Conductor actualizado", type: "success" });
  ...
  const handleDelete = () => {
    ...
    showAlert({
      ...
      message: "¿Eliminar este conductor? Esta acción no se puede deshacer.",
      type: "warning",
      buttons: [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            setLoading(false);
            if (result.success) {
              setToast({ visible: true, message: "Conductor eliminado", type: "success" });
              setTimeout(() => router.back(), 1500);
            } else {
        ...
        >
          {loading ? (
            ...
            <>
              ...
              <Text style={{ color: "#fff", fontWeight: "700", fontSize: 16, marginLeft: 8 }}>Actualizar Conductor</Text>
            </>
          )}
        </TouchableOpacity>
      </KeyboardAwareScrollView>
